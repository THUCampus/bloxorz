...
class GameView {
    ...
    computeScore () {
        let best: number = this.gate_info['best_moves'][this.gate_index];
        let score: number;
        if (this.parent.stepcount <= best) {
            score = 3;
        } else if (this.parent.stepcount <= best + 15) {
            score = 2;
        } else if (this.parent.stepcount <= best + 50) {
            score = 1;
        } else {
            score = 0;
        }
        if (score > this.gate_info['levels'][this.gate_index]) {
            this.gate_info['levels'][this.gate_index] = score;
        }
    }
    updatePassedLevel(){
        let count:number = 0;
        console.log(this.gate_info['levels']);
        for(let i:number = 1; i <= 23; i++)
        {
            if(this.gate_info['levels'][i] > 0)
            count += this.gate_info['levels'][i];
        }

        this.gate_info['passed_level'] = count;
        let opendata = wx.getOpenDataContext();
        opendata.postMessage({type: 'update', score:count.toString()});
    }
    unlockNextLevel () {
        this.gate_index += 1;
        if (this.gate_info['levels'][this.gate_index] === -1) {
